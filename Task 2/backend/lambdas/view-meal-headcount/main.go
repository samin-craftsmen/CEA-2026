...
type ViewMealHeadcountRequest struct {
	AdminDiscordID string `json:"admin_discord_id"`
	Date           string `json:"date"`
}

func ViewMealHeadcountHandler(request events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	var req ViewMealHeadcountRequest
	if err := json.Unmarshal([]byte(request.Body), &req); err != nil ||
		req.AdminDiscordID == "" || req.Date == "" {
		return jsonResponse(http.StatusBadRequest, map[string]string{
			"error": "invalid request: admin_discord_id and date are required",
		}), nil
	}

	result, err := service.AdminGetHeadcountSummary(req.AdminDiscordID, req.Date)
	if err != nil {
...
func main() {
	...
	lambda.Start(ViewMealHeadcountHandler)
}
